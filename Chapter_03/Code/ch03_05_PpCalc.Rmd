...
note: This script includes Pp analysis using selected datasets from Reanalysis4 (df_REa6)
---
...
df_1 <-read.table(paste0(workDPath,"df_Rea6.txt"),header=TRUE)
...
df_Gr <-read.table(paste0(rawPath,"CultivarGroup.txt"),header=TRUE)
...
##change name of colum treat cultivar for Cultivar, adjust formats dates as dates

```{r}
df_Rea7 <-df_Rea7 %>%
   rename(Cultivar=Treatcultivar) %>%
...
df_Rea7 %>%
  ...
  ggplot(aes(x=month(SowingDate), y= Value, colour=Cultivar)) +
...
df_Rea7 %>%
  ...
  ggplot(aes(x=reorder(Month,month(SowingDate)), y= Value, colour=Cultivar))+ 
  geom_point(shape=21) +
...
Sense Group was developed based on Nicols 2013 creteria

to make the x axis correct the sowind datees need to be transformed as the same year to be able to produce a geom smooth graph with months names
...
startdate<- dmy("1/1/2000")

df_Rea7 %>%
rowwise()%>%
mutate (DOY=SowingDate-as.Date(dmy(paste0("1/1/",year(SowingDate)))))%>%
mutate(DOY=as.numeric(DOY))%>%
  mutate(DateCommon=startdate+DOY) %>%
  mutate(Variable=factor(Variable, level=c("DaysToFlower","TTFloAdj"),
                         labels=c("Days to flower","Thermal-time (oCd)"))) %>%
   ggplot(aes(x=as.Date(DateCommon,"%b"), y= Value,colour=Cultivar))+
  geom_point(aes(shape = factor(SensGroup),colour=Cultivar),size=2.5)+
  labs(x=" Sowing month", y="Time fo 50% flowering")+
    theme_bw()+
 geom_smooth(se=TRUE, aes(colour=SensGroup, fill=SensGroup))+
...
ggsave(paste0(workDPath,"ch03_05_DaysTTFlower_SowingMonth_subset.tiff"),width=12, height=10,dpi = 300)
...
df_weather <-read.table(paste0(workDPath,"df_CumTT.txt"),header=TRUE)
...
Now bring data vernalization from Reanalysis scripts 8 and 7  
...
df_temp <-read.table(paste0(workDPath,"df_tempVern.txt"),header=TRUE)
df_tempVern <- df_temp %>%
