...
#get dataframe created in ReAnalysis script 1
...
write.table(df_Rea3,paste0(workDPath, "df_Rea3.txt"))
...
library(broom)
df_stats <- tidy(df_ReaSlop, mod) %>%
  mutate(term=factor(term)) %>%
  filter(term=="TTtoFlower")
...
df_stats %>%
  group_by(SowingDate) %>%
  ggplot(aes(x=reorder(Treatcultivar,estimate), y=95/estimate,fill=Treatcultivar)) +
...
df_tempTTflo <- df_stats %>%
  mutate(TTflower= 95/estimate)

#write.table(df_tempTTflo, "dfTempTTFLOR.txt")
write.table(df_tempTTflo,paste0(workDPath, "dfTempTTFLOR.txt")) # FIXME: data need to be saved with clear/EXPLANATORY NAME .... NOT only a NUMBER!!!!!!!
...
# df_ReaSlop %>%
#   group_by(SowingDate)%>%
#   mutate(TT=95/estimate)%>%
#   summarise(mean=mean(estimate), sd=sd(estimate),meanTT=mean(TT), sdTT=sd(TT))

df_tempTTflo1 <-df_tempTTflo %>%
 group_by(SowingDate)%>%
  dplyr::select(TTflower)%>%
  summarise_all(funs(mean,sd))
  
  
# calculate Standard error as  <- function(x) sd(x)/sqrt(smaple size(n)) for dear et al n=4
summary(df_tempTTflo1)
```
